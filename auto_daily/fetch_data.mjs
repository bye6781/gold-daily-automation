...
// ========== 5. 新闻摘要 ==========
  // 金十新闻过滤：优先黄金、美联储、地缘相关新闻
  // 金十新闻过滤：优先黄金、美联储、地缘相关
  const goldKeywords = /黄金|金价|美联储|利率|央行|PCE|CPI|通胀|地缘|中东|美元|ETF|COMEX|债券|收益率|原油|降息|加息/;
  const goldNews = jin10Data.news.filter(s => goldKeywords.test(s));
  // 优先金十重要事件（通常与市场相关），再取金十新闻，最后兜底
  const topNewsSources = [
    ...(jin10Data.importantEvents || []).filter(s => goldKeywords.test(s) || s.length < 60),
    ...goldNews,
    ...jin10Data.news
  ];
  const topNews = [...new Set(topNewsSources)].slice(0, 3)
    .map(s => s.replace(/[。！？]$/, ""))
    .join("；")
    || "国内金饰价格随国际金价波动，今日品牌金饰约" + jewelryPrice + "元/克";
...
if (dNum > 101) fedImpact = '美元偏强（' + dNum.toFixed(1) + '），对黄金构成压制。若经济数据疲软或降息预期升温，美元回落将为金价提供反弹动力。';
  ...
  // FED_ANALYSIS: 基于美元指数和金十重要事件
  var fedAnalysis;
  const fedEvents = (jin10Data.importantEvents || []).filter(e => /美联储|利率|PCE|CPI|鲍威尔|通胀|降息|加息/.test(e));
  if (fedEvents.length > 0) {
    fedAnalysis = fedEvents.slice(0,2).join("；");
  } else if (dNum > 102) {
    fedAnalysis = "美元指数走高至" + dNum.toFixed(1) + "，美联储鹰派立场压制金价。关注后续经济数据能否软化联储态度。";
  } else {
    fedAnalysis = "美联储6月维持利率不变，释放鹰派信号。市场关注后续PCE数据对降息时点的指引。";
  }

  // CB_ANALYSIS: 全球央行购金分析
  const cbEvents = (jin10Data.importantEvents || []).filter(e => /央行|购金|储备|黄金ETF/.test(e));
  var cbAnalysis;
  if (cbEvents.length > 0) {
    cbAnalysis = cbEvents.slice(0,1).join("");
  } else {
    cbAnalysis = "全球央行购金意愿维持高位，2025年净购金量超1000吨。新兴市场央行持续多元化储备，为金价提供中长期底部支撑。";
  }

  // GEO_ANALYSIS: 地缘因素分析
  const geoEvents = (jin10Data.importantEvents || []).filter(e => /伊朗|中东|美伊|停火|谈判|黎巴嫩|胡塞|红海|俄乌|地缘/.test(e));
  var geoAnalysis;
  if (geoEvents.length > 0) {
    geoAnalysis = geoEvents.slice(0,2).join("；");
  } else {
    geoAnalysis = "地缘风险溢价阶段性收窄，但中东局势仍存不确定性，随时可能引发避险买盘。";
  }
